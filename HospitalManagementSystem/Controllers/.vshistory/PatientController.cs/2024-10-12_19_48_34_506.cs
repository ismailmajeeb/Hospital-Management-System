...
namespace HospitalManagementSystem.Controllers
{
    public class PatientController : Controller
    {
        ...
        //[ValidateAntiForgeryToken]
        [Authorize(Roles = SD.Patient)]
        public async Task<IActionResult> Appointments(string userId)
        {
            var patient =  await unitOfWork.Patients.FindAsync(x => x.UserId == userId);
            if (patient == null) return View("Error");
            var temp = await unitOfWork.Appointments.FindAllAsync(a=>a.PatientId == patient.Id);
            List<PatientAppointmentsModel> model = temp.Select(a => new PatientAppointmentsModel
            {
                AppointmentId = a.Id,
                DateTime = a.DateTime,
